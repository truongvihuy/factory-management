...
export class AuthClient extends BaseClient {
  ...
  async login(token: string, options?: RequestContext) {
    return this._requestServer('post', `${this.prefixUrl}/auth/login`, { token }, options);
  }

  async verify(token: string, options?: RequestContext) {
    return this._requestServer('post', `${this.prefixUrl}/auth/verify`, { token }, options);
  }

  async getUser(userId: string, options?: RequestContext) {
    return this._requestServer('get', `${this.prefixUrl}/user/${userId}`, null, options);
  }

  async createUser(user: User, options?: RequestContext) {
    return this._requestServer('post', `${this.prefixUrl}/user`, user, options);
  }

  async updateUser(userId: string, user: User, options?: RequestContext) {
    return this._requestServer('put', `${this.prefixUrl}/user/${userId}`, user, options);
  }

  async checkAdmin(userId: string, options?: RequestContext) {
    return this._requestServer('get', `${this.prefixUrl}/user/admin-permission/${userId}`, null, options);
  }

  async updateAdmin(userId: string, admin: boolean, options?: RequestContext) {
    return this._requestServer('post', `${this.prefixUrl}/user/admin-permission/${userId}/${admin}`, null, options);
  }

  async getPemissions(userId: string, options?: RequestContext) {
    return this._requestServer('get', `${this.prefixUrl}/user/permission/${userId}`, null, options);
  }

  async checkPermission(userId: string, factoryId: string, role: Role, options?: RequestContext) {
    return this._requestServer(
      'get',
      `${this.prefixUrl}/user/permission/${userId}/${factoryId}/${role}`,
      null,
      options,
    );
  }
  ...
  async getPermission(userId: string, factoryId: string, options?: RequestContext) {
    return this._requestServer('get', `${this.prefixUrl}/user/permission/${userId}/${factoryId}`, null, options);
  }
  ...
  async deletePermission(userId: string, factoryId: string, options?: RequestContext) {
    return this._requestServer('delete', `${this.prefixUrl}/user/permission/${userId}/${factoryId}`, null, options);
  }
}
